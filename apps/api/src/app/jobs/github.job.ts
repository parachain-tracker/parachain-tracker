            ...
            if (github_repo) {
                ...
                setTimeout(async () => {
                    try {
                        ...
                        const res_last = await await this.httpService.get(last.url, {headers}).toPromise()
                        const items_last = res_last.data.length;

                        const commits = items_per_page * (parseInt(last.page) - 1) + items_last 

                        res = await await this.httpService.get(`https://api.github.com/search/repositories?q=repo:${github_repo}`, {headers}).toPromise()
                        const stargazers = res.data.items[0].stargazers_count
                
                        await projectsRepo.update(projects[i].id, { commits: commits, stars: stargazers})
                        console.log(`Successfully updated commits and stars for PROJECT: ${projects[i].name} - commits: ${commits} - stars: ${stargazers}`)
                    } catch(e) {
                        console.log(`Failed to update commits and stars for PROJECT: ${projects[i].name} - error: ${e}`)
                    }
                }, i * 18100)  
            } else {
